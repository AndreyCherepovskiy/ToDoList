...
function ListTask() {
    ...
    this.isEmptyListTaskByAssignee = function () {
        for(var i = 0; i < this.collection.length; i++){
            if(this.collection[i].assignee.login === dataModel.currentUser.login){
                return false;
            }
        }
        return true;
    ...
    this.isEmptyListTaskByAuthor = function () {
        for(var i = 0; i < this.collection.length; i++){
            if(this.collection[i].author.login === dataModel.currentUser.login &&
                this.collection[i].assignee.login != dataModel.currentUser.login){
                return false;
            }
        }
        return true;
